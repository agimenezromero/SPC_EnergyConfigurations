...
class EnergyConfigs_AB(object):
	"""docstring for EnergyConfigs"""
	def __init__(self, m_A, m_B, d, adimensionalised):
		
		self.m_A = m_A #Magnetic dipole moment A
		self.m_B = m_B #Magnetic dipole moment B
		...
		self.U_BB = (self._mu_0 * self.m_B**2) / (2 * np.pi * self.d**2)
		self.U_AB = (self._mu_0 * self.m_A * self.m_B) / (2 * np.pi * self.d**2)

		if adimensionalised == True:
			self.d = 1
			self.U_AB = 1

	def U(self, U, r, theta):
		return -U * (self.d / r)**3 * (1 - 1.5 * np.sin(theta)**2)
...
class MakePlot(object):
	def __init__(self, title, xlabel, ylabel, data_labels, avg_labels, grid, data_linestyles, avg_linestyles, data_colors, avg_colors, data_markers, plot_avg, default, legend, equil, right, left, top, bottom, wspace, hspace, width, height, background, borders):
...
if __name__ == "__main__":
	...
	energies = EnergyConfigs(m, d, adimensionalized)

	r, E = energies.two_identical_chains_linear(N0, Nf)
